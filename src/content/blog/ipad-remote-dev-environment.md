...
요즘 근무 환경과 생활 패턴 상 아이패드를 들고 다니면서, 집에 있는 맥북에 원격으로 붙어서 개발하는 일이 잦아졌다. 처음엔 GitHub Desktop이나 VS Code 웹 버전으로 아이패드에서 바로 작업해보려 했는데, 파일 시스템 접근이 제한적이고 터미널 조작이 부자연스러운 데다 확장 기능도 데스크톱 버전만큼 자유롭지 못했다. 결국 아이패드를 에디터로 쓰는 게 아니라 아이패드를 맥북 원격 접속 툴로 사용하기로 했고, 그 결과 Blink Shell + Tailscale + tmux + nvim 조합으로 아이폰/아이패드/맥북에서 개발 중이다.
